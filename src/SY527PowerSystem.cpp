...
SY527HVChannel::SY527HVChannel(uint32_t address, HVBoard& board, uint32_t id, CaenetBridge* bridge):HVChannel(address,board,id,bridge),status_(0) {}
...
void SY527HVChannel::setSoftMaxV(uint32_t) {
...
void SY527HVChannel::setStatus(std::vector<uint32_t>::const_iterator data) {
  vmon_   = *data++;
  imon_   = *data++;
  maxV_   = *data++;
  status_ = *data++;
...
SY527PowerSystem::SY527PowerSystem(uint32_t address, CaenetBridge* bridge):HVModule(address,bridge),status_(0) { 
...
void SY527PowerSystem::discoverBoards() {
  ...
  boards[0] = HVBoard(0,"SY527", 2, 0, 0, 4, 8000, 3000, 0, 500, 500, 1000, 0, 0);
}
...
void SY527PowerSystem::updateStatus() {
  ...
  for(auto [key,channel] : getChannels()) {
    static_cast<SY527HVChannel*>(channel)->setStatus(data.begin()+(channel->id()*4));
  }
}

void SY527PowerSystem::kill() {
  // send command
...
void SY527PowerSystem::enableKeyboard(bool enable) {
